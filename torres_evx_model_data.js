export const torresEvxModels = [
  {
    name: "Torres Evx 4x2",
    slug: "torres_evx_4x2",
    image: require("./torres_evx_ana_gorsel.png"),
    desc_item: [
      "Otomatik",
      "Elektrik",
      "4x2",
    ],
    desc: "₺1.880.000'den başlayan fiyatlarla",
    technic_properties: [
      {
        name: "Genel Bakış",
        props: [
          {
            name: "Kapı Sayısı",
            value: "5 Kapı",
          },
          {
            name: "Azami Sürat",
            value: "175 km/saat",
          },
          {
            name: "Vites Sayısı",
            value: "6 Vites",
          },
        ],
      },
      {
        name: "Motor ve Performans",
        props: [
          {
            name: "Motor Gücü",
            value: "152,2 kw (206,9) hp",
          },
          {
            name: "Maksimum Tork",
            value: "339 Nm",
          },
        ],
      },
      {
        name: "Yakıt Tüketimi",
        props: [
          {
            name: "Ortalama Enerji Tüketimi",
            value: "13,5 Wh/Km",
          },
          {
            name: "Batarya Kapasitesi	",
            value: "73,4 kWh",
          },
        ],
      },
      {
        name: "Boyutlar",
        props: [
          {
            name: "Koltuk Sayısı",
            value: "5 Koltuk",
          },
          {
            name: "Uzunluk",
            value: "4715 mm",
          },
          {
            name: "Genişlik",
            value: "1890 mm",
          },
          {
            name: "Yükseklik",
            value: "1715 mm",
          },
          {
            name: "Net Ağırlık",
            value: "1915 kg",
          },
          {
            name: "Bagaj Hacmi",
            value: "839 lt",
          },
          {
            name: "Lastik Ölçüleri",
            value: "225 R60 - 245 R45",
          },
        ],
      },
    ],
    hardware_properties: [
      {
        name: "Güvenlik",
        props: [
          "ARP (Aktif Devrilme Önleyici)",
          "HSA (Yokuş Kalkış Sistemi)",
          "HDC (Yokuş İniş Kontrolü)",
          "TPMS (Lastik Basıncı İzleme Sistemi)",
          "Ön Emniyet Kemeri Yük Sınırlama Düzeneği ve Kilitleme Dilli Ön Gergiler",
          "Hızı Algılayan Otomatik Kapı Kilitleri",
          "Hıza Duyarlı Emniyet Kemeri Uyarısı",
          "AEBS (Bağımsız Acil Fren Sistemi)",
          "LDW ( Şeritten Çıkış Uyarısı)",
          "LKA ( Şerit Koruma Asistanı)",
          "HBA (Uzun Far Asistanı)",
          "SDA (Emniyetli Mesafe Uyarısı)",
          "DAA (Sürücü Yorgunluk Uyarısı)",
          "FVSA (Öndeki Araç İçin Hareket Uyarısı)",
          "Takviyeli Monoblok Gövde Yapısı",
          "Yan Darbelere Karşı Kapı Darbe Kirişleri",
          "Alarm",
          "ESS (Acil Durum Arka Sinyal Sistemi)",
          "ESP (Elektronik Stabilite Programı)",
          "ABS (Kilitlenme Karşıtı Fren Sistemi)",
          "TCS (Çekiş Kontrol Sistemi)",
          "HBA (Hidrolik Fren Asistanı)",
          "Immobilizer",
          "Sürücü ve Yolcu Ön Hava Yastıkları",
          "Sürücü ve Yolcu Yan Hava Yastıkları",
          "Sürücü Perde Hava Yastığı",
          "Ön Yolcu Perde Hava Yastığı",
          "Ön Yolcu Hava Yastığı Kapama Düğmesi",
          "Önde Hava Soğutmalı Disk Frenler",
          "Arkada Disk Frenler",
          "Yükseklik Ayarlı Ön Emniyet Kemerleri",
          "Arka Koltuklarda Üç Noktadan Bağlantılı Emniyet Kemer Sistemi",
          "Çocuk Güvenlik Kilidi",
          "Hıza Duyarlı Kapı Kilidi",
          "Sürücü Diz Hava Yastığı",
          "Isofıx Çocuk Koltuğu Bağlantısı",
          "FTCS-Çekiş Kontrol Sistemi",
          "FCW-Ön Çarpışma Uyarısı",
          "RCTA-Arka Çapraz Trafik Uyarısı",
          "Arka Yolcu Perde Hava Yastıkları",
          "Park Asistanı Sistemi",
          "Yüksek Gergili Çelik Yapı",
          "Darbe Emici Özellikli Kapı Kirişleri",
          "BSD-Kör Nokta Uyarı Sistemi",
          "İlk Yardım Kiti",
        ],
      },
      {
        name: "Multimedia",
        props: [
          "Araç Bağlantısı (Apple Carplay/Google Android Auto)",
          "6 Adet Hoparlör",
          "Bluetooth Bağlantı",
          "USB Bağlantısı",
        ],
      },
      {
        name: "Konfor",
        props: [
          "Yeni Tip Çok Fonksiyonlu Anahtar (Smart-Key)",
          "Elektronik Far Yükseklik Ayarı",
          "Kabin İçi Hava Kalite Kontrol Sistemi",
          "Arka Koltuk Klima Ünitesi-Manuel",
          "Isıtmalı Arka Koltuklar",
          "Isıtmalı ve Soğutmalı Ön Koltuklar",
          "Bardak Tutuculu Arka Kol Dayama",
          "60/40 Katlanabilir ve Yatırılabilir 2.Sıra Koltuklar",
          "8 Yönlü Elektrikli Koltuk",
          "Anahtarsız Giriş ve Çalıştırma (Start & Stop)",
          "Çift Bölgeli İyonizerli Otomatik Klima",
          "Çok Bağlantılı Spiral Arka Süspansiyon",
          "Bardak Tutucu (Ön ve Arka)",
          "Isıtmalı Ön Koltuklar",
          "Işıklı Makyaj Aynası",
          "Yükseklik Ayarlı Sürücü Koltuğu",
          "McPherson Spiral Ön Süspansiyon Sistemi",
          "EPS (Elektrik Destekli Direksiyon)",
          "Yağmur Sensörü",
          "Elektronik Park Freni (Otomatik Tutmalı)",
          "Sürüş Modları Sistemi (Normal/Kış/Spor)",
          "Lastik Tamir Kiti",
          "Smart Key (Akıllı Anahtar-Anahtarsız Giriş/Çalıştırma/Kilitleme)",
          "Kablosuz Şarj",
          "Cruise Control (Hız Sabitleme Sistemi)",
        ],
      },
      {
        name: "Dış Donanım",
        props: [
          "Ses Yalıtımlı Renkli Ön Cam",
          "LED Işıklı Gündüz Farları",
          "Renkli Yan Camlar",
          "Arkada Sportif Koruyucu Plakası",
          "Elektrikli, Katlanır ve Isıtmalı Dikiz Aynaları",
          "Ön Kapı Aydınlatma Lambası",
          "Elektrikli Güvenlikli Sürücü Camı",
          "Elektrikli Güvenlikli Sürücü Camı",
          "Gövde Rengi Kapı Kolları",
          '20" Aluminyum Alaşım Jant',
          "Ön Sis Farları",
          "5 Noktadan Bağlantılı Bağımsız Arka Süspansiyon",
          "Tavan Çıtaları",
          "Köpekbalığı Anten",
          "Ön ve Arka Park Sensörü",
          "Elektrikli Bagaj Kapağı",
          "Geri Görüş Kamerası ve Park Kılavuz Çizgileri",
          "Sesi Azaltan Akustik Motor Kapağı",
          "Karartılmış Arka Camlar",
          "LED Arka Stop",
          "Aralıklı Arka Cam Sileceği",
          "Görsel VIN (Şasi Numarası)",
          "Dış Aynalarda Led Sinyal Lambaları",
          "Siyah Hava Giriş Izgarası",
          "Arka Sis Farları",
          "Sunroof",
        ],
      },
      {
        name: "İç Donanım",
        props: [
          "Işığa Duyarlı Elektrokrom İç Dikiz Aynası",
          "Manuel Eğim Kontrollü Teleskopik Direksiyon",
          "Ön Konsol Arkasında 12V Güç Çıkışı ve USB Şarj",
          "LED Kabin İçi Aydınlatma",
          "12V Güç Soketi",
          "Deri Kaplama Direksiyon",
          "Gümüş İç Kapı Kolları",
        ],
      },
    ],
  },
];

export const torresEvxPrices = [
  {
    model: "Torres EVX 4x2",
    price: "1.880.000",
  },
];

export const torresEvxColors = [
  {
    name: "Amazon Yeşili",
    image: require("./arac_gorseller/torres_evx/torres_evx_amazon_yesili.png"),
  },
  {
    name: "Beyaz",
      image: require("./arac_gorseller/torres_evx/torres_evx_beyaz.png"),
  },
  {
    name: "Demir Gri",
    image: require("./arac_gorseller/torres_evx/torres_evx_demir_gri.png"),
  },
  {
    name: "Latte Beji",
    image: require("./arac_gorseller/torres_evx/torres_evx_latte_beji.png"),
  },
  {
    name: "Mavi",
    image: require("./arac_gorseller/torres_evx/torres_evx_mavi.png"),
  },
  {
    name: "Platin Gri",
    image: require("./arac_gorseller/torres_evx/torres_evx_platin_gri.png"),
  },
  {
    name: "Siyah",
    image: require("./arac_gorseller/torres_evx/torres_evx_siyah.png"),
  },
];

export const torresEvxGallery = [
  require("./kgm-uygulama-dosyalar/gorsel/torres_evx_gallery1.png"),
  require("./kgm-uygulama-dosyalar/gorsel/torres_evx_gallery2.png"),
  require("./kgm-uygulama-dosyalar/gorsel/torres_evx_gallery4.png"),
  require("./kgm-uygulama-dosyalar/gorsel/torres_evx_gallery5.png"),
  require("./kgm-uygulama-dosyalar/gorsel/torres_evx_gallery6.png"),
  require("./kgm-uygulama-dosyalar/gorsel/torres_evx_gallery7.png"),
  require("./kgm-uygulama-dosyalar/gorsel/torres_evx_gallery8.png"),
  require("./kgm-uygulama-dosyalar/gorsel/torres_evx_gallery9.png"),
  require("./kgm-uygulama-dosyalar/gorsel/torres_evx_gallery10.png"),
  require("./kgm-uygulama-dosyalar/gorsel/torres_evx_gallery11.png"),
  require("./kgm-uygulama-dosyalar/gorsel/torres_evx_gallery12.png"),
  require("./kgm-uygulama-dosyalar/gorsel/torres_evx_gallery13.png"),
];

export const torresEvxVideo = [
  {
    cover: require("./kgm-uygulama-dosyalar/gorsel/torres_evx_video_cover.png"),
    video: require("./kgm-uygulama-dosyalar/video/torres_evx_video.mp4"),
  },
  {
    cover: require("./kgm-uygulama-dosyalar/gorsel/torres_evx_video_cover3.png"),
    video: require("./kgm-uygulama-dosyalar/video/torres_evx_video3.mp4"),
  },
  {
    cover: require("./kgm-uygulama-dosyalar/gorsel/torres_evx_video_cover4.png"),
    video: require("./kgm-uygulama-dosyalar/video/torres_evx_video4.mp4"),
  },
]